...
- the first milestone supports only the narrow RTMP publish flow needed by common publishers such as FFmpeg
- the first WHIP milestone supports H264 video and Opus audio from default WebRTC offers and rejects other codecs
- the first WHIP milestone uses host ICE candidates only and is intended for local or directly reachable publishers
- playback verification is done through the generated HLS output, not RTMP play support
- the first HLS sink uses libavformat/libavcodec/libavutil in-process and remuxes the inbound FLV stream into HLS without re-encoding
- the WebRTC HLS path writes fMP4 HLS segments directly from depacketized packets without re-encoding
- live HLS mode currently uses a 3-entry sliding playlist and keeps 3 extra stale `.ts` segments before the sink cleaner removes them
- the current live TS packet-mode path assumes H264 video and AAC audio elementary streams; Opus handling is deferred to a later layer
- RTMP protocol handling is implemented in-house but organized with explicit handshake, chunk, command, and session boundaries inspired by `nginx-rtmp-module`
- the first `ISource` step is metadata-and-lifecycle only; packet-pull semantics are deferred to the next source abstraction pass
- during the runtime transition, the CLI bootstrap path may still predeclare source-to-sink routes before a publisher connects, while the future `upsert_manifest` runtime control path will validate sink bindings against currently active sources
- the first decoder step supports H264 only, ignores audio, and auto-detects FLV-tag vs packetized source payloads
- the first compose step writes `frame_000050.jpg`, `frame_000100.jpg`, ... every `snapshot_interval` decoded frames (default `50`) to the compose `output_root`
- `side_by_side` compose writes `scene_000050.jpg`, ... as a doubled-width JPG with the same frame cloned left and right
- `clip_prompt` compose runs MobileCLIP2-S2 through ONNX Runtime, logs `score` (cosine similarity to `prompt`) and `latency_ms` to stderr and `./infer/infer.log`
- compose requires a decoder; the CLI auto-creates one when `--compose`, `--compose-side-by-side`, or `--compose-infer-prompt` is used without `--decoder`
- FFmpeg RTMP publishers may include a 3-byte composition-time field before avcC in FLV AVC sequence headers; the decoder skips that when extracting extradata
